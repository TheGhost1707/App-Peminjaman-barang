...
<body>
                  ...
                  <tbody>
                    ...
                    if ($result->num_rows > 0) {
                      ...
                      while ($row = $result->fetch_assoc()) {
                        ...
                        // Bagian untuk tombol Approve
                        echo "<td>";
                        if ($row['status'] == 'pending') {
                          echo "<a href='approve_loan.php?loan_id=" . $row['loan_id'] . "&approved=true' class='btn btn-success btn-sm'>Approve</a>"; // Tombol Approve jika statusnya 'pending'
                        } else {
                          echo "Approved"; // Jika sudah di-approve
                        }
                        echo "</td>";

                        // Bagian untuk tombol Return
                        echo "<td>";
                        if ($row['status'] == 'borrowed' && empty($row['return_date'])) {
                          echo "<button type='button' class='btn btn-warning btn-sm' data-bs-toggle='modal' data-bs-target='#returnModal" . $row['loan_id'] . "'>Return</button>";

                          // Modal untuk input barang bagus dan rusak
                          echo "
        <div class='modal fade' id='returnModal" . $row['loan_id'] . "' tabindex='-1' aria-labelledby='returnModalLabel' aria-hidden='true'>
            ...
            <div class='modal-content'>
              ...
              <div class='modal-body'>
                <form action='process_return.php' method='POST'>
                  ...
                  <div class='mb-3'>
                    <label for='available_quantity' class='form-label'>Good Quantity</label>
                  ...
                  <div class='mb-3'>
                    ...
                    <input type='number' class='form-control' name='damaged_quantity' id='damaged_quantity' required>
                  </div>
                  ...
                  <div class='modal-footer'>
                    ...
                    <button type='submit' class='btn btn-primary'>Submit</button>
                          ...
                          echo "Returned"; // Jika sudah dikembalikan, tampilkan teks ini
                        }
                        echo "</td>";
                      ...
                      echo "<tr><td colspan='11' class='text-center'>No loan requests found</td></tr>";
                    }
                ...
                <div class="mt-4">
                  <!-- Tombol untuk mengarah ke print.php -->
                  <a href="print.php" class="btn btn-primary">Print Request List</a>
                </div>
              </div>
            </div>
          </div>
  ...
  <script>
    // Cek apakah ada parameter 'approve_success' atau 'approve_error' di URL
    const urlParams = new URLSearchParams(window.location.search);

    // Jika approve_success=true, tampilkan pesan sukses
    if (urlParams.has('approve_success') && urlParams.get('approve_success') === 'true') {
      alert('Approval successful! The request has been approved.');
    }

    // Jika approve_error=insufficient_stock, tampilkan pesan error
    if (urlParams.has('approve_error') && urlParams.get('approve_error') === 'insufficient_stock') {
      alert('Approval failed! Insufficient stock available.');
    }
  </script>
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      // Cek apakah ada parameter 'return' di URL
      const urlParams = new URLSearchParams(window.location.search);

      // Debugging: log URL parameters
      console.log("URL Params:", urlParams.toString());

      // Jika return=success, tampilkan pesan sukses
      if (urlParams.has('return') && urlParams.get('return') === 'success') {
        alert('Return process successful!');
      }

      // Jika return=error, tampilkan pesan error dengan rincian pesan kesalahan
      if (urlParams.has('return') && urlParams.get('return') === 'error') {
        let errorMessage = urlParams.has('message') ? decodeURIComponent(urlParams.get('message')) : 'An error occurred during the return process.';
        alert('Return process failed! ' + errorMessage);
      }
    });
  </script>
